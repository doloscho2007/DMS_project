...
export default function FileView() {
  ...
  const [showLoaderCheckinFolder, setShowLoaderCheckinFolder] = useState(false);
  const [showLoader1CheckinFolder, setShowLoader1CheckinFolder] =
    useState(false);
  ...
  const handleCheckinClose = () => {
    setShowLoaderCheckinFolder(false);
    setShowLoader1CheckinFolder(false);
  ...
  const getBookmarkStatus = useCallback(async () => {
    const resp = await DMSAPI.getBookmarkStatus(newFileId);
  ...
  const getCheckoutStatus = useCallback(async () => {
    const resp = await DMSAPI.getCheckoutStatus(newFileId);
    const data = await resp.json();
    // console.log(resp);
    // console.log(data.message);
  ...
  function performCut() {
    ...
    localStorage.setItem(
      "perfCutCopyID",
      stringResponse === "" || null || undefined ? newFileId : stringResponse
    );
  }
  // CUT METHOD END
  // COPY METHOD START
  ...
  const addBookmarkMeth = async () => {
    const response = await DMSAPI.addBookmark(stringResponse);
  ...
  const handleCheckin = async (e) => {
    setShowLoaderCheckinFolder(true);
    e.preventDefault();
    ...
    if (selectedFiles.length === 0) {
      ...
      try {
        ...
        if (response.status === 200) {
          handleCheckinClose();
          setSnackbarMessage("File Checkin Successful");
          setSnackbarSeverity("success");
          setOpen(true);
          setShowLoaderCheckinFolder(false);
          setShowLoader1CheckinFolder(true);
          setTimeout(() => {
            handleRefresh();
          }, 2000);
        } else if (
  ...
  return (
    ...
    >
      <div>
        {listView ? (
          <Paper sx={{ width: "100%", mb: 2, height: "60%" }}>
            <TableContainer
              sx={{ maxHeight: 440 }}
              // component={Paper}
            >
        ...
        <div>
          <ContextMenu top={points.y} left={points.x}>
            <ul>
              ...
              <li
                className={
                  // bookMarkStatus === "Bookmark Not Present"
                  //   ? ""
                  //   : checkOutOpen
                  //   ? "disabled"
                  //   : ""
              ...
              <li
                ...
                onClick={deleteBookmarkMeth}
              >
                Delete Bookmark
              </li>
              <li
                className={
                  checkOutStatus === "File is not checked out."
                    ? ""
                    : "disabled"
                }
                onClick={addCheckout}
      ...
      >
        ...
        <DialogContent dividers>
          ...
          >
            ...
            <div>
              {showLoaderCheckinFolder === true &&
              showLoader1CheckinFolder === false ? (
                <div>
                  <h1>Loading...</h1>
                </div>
              ) : (
                <>
                  {showLoaderCheckinFolder === false &&
                  showLoader1CheckinFolder === false ? (
                    <h1></h1>
                  ) : (
                    <h1> File Added</h1>
