...
const SearchView = () => {
  const { setSearchFiles } = useGlobalContext();
  ...
  const [searchByCreatedDate2Value, setSearchByCreatedDate2Value] =
    useState("");
  const [checkWholeWordValue, setCheckWholeWordValue] = useState(false);
  // let searchResult=[];
  let resJson;
  ...
  const handleCheckWholeWordValue = (e) => {
    ...
    setCheckWholeWordValue(!checkWholeWordValue);
  };
              ...
              return (
                <option
                  value={folder.folderPath
                    .substring(1, folder.folderPath.length - 1)
                    .replaceAll(" ", "-")}
                >
                  {folder.folderPath
                    .substring(1, folder.folderPath.length - 1)
